...
partial struct ValueStack<T> {
    ...
    public readonly T First(Func<T, bool> predicate) {
        ...
        throw new Exception("No elements in the value stack match the predicate.");
    }
    ...
    /// <summary>
    /// Ensures the stack has exactly one element and returns that element.
    /// </summary>
    /// <exception cref="IndexOutOfRangeException"/>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public readonly T Single() {
        if (Count != 1) {
            throw new IndexOutOfRangeException("The value stack does not contain exactly one element.");
        }
        return Buffer[0];
    }

    /// <summary>
    /// Ensures the stack has exactly one element matching <paramref name="predicate"/> and returns that element.
    /// </summary>
    /// <exception cref="Exception"/>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public readonly T Single(Func<T, bool> predicate) {
        ...
        for (int index = Count - 1; index >= 0; index--) {
            ...
            if (predicate(element)) {
                if (found) {
                    throw new IndexOutOfRangeException("The value stack contains more than one element matching the predicate.");
                }
                found = true;
                result = element;
            }
    ...
    /// <summary>
    /// Ensures the stack has exactly one element and returns that element or the default value.
    /// </summary>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    /// <summary>
    /// Ensures the stack has exactly one element and returns that element matching <paramref name="predicate"/> or the default value.
    /// </summary>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
